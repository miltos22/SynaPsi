...
3. **Alternate architectures** — heterogeneous, non-LLM computation engines to which decidable spans are routed: simulators, solvers, checkers, calculators — and non-LLM neural networks that predict the world or physics (learned dynamics / world models in a non-LLM format). The result of a routed span is part of the same cognitive process as the generator's own output.
...
5. **Knowledge library** — a knowledge database: a pre-compiled, separately-trained expert dedicated to keeping track of the model's overall knowledge, requested regularly during generation.
