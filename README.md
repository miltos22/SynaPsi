...
4. **Critic expert** — a component specialized for critique, **trained separately from the generator**, so that its independence is structural rather than nominal. It returns a verdict: accept / regenerate / abstain.
5. **Knowledge oracle** — a calibrated coverage probe that determines whether the system can be trusted on a given topic, powering the abstention decision.
6. **Continuous sentiment memory** — per-user; accumulates polarity over time. It stores no interpretations, only sentiment. The reasoning component interprets; the critic audits.
